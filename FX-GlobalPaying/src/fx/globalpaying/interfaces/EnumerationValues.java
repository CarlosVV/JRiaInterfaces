...
public class EnumerationValues {
    ...
    public static GetEnumerationValuesRequestEntity parseInputArgsToRequest(String[] args) {
        GetEnumerationValuesRequestEntity request = new GetEnumerationValuesRequestEntity();
        ...
        if (args[3] != null) {
            ...
            if (requestArray.length >= 3) {
                ...
                enumType = requestArray[1];
                countryFrom = requestArray[2];
            }
        }
        request.setRequestType(requesType);
        request.setEnumType(enumType);
        request.setCountryFrom(countryFrom);

        return request;
    }

    private static void createSoapEnvelope(SOAPMessage soapMessage,
            GetEnumerationValuesRequestEntity request) throws SOAPException {
        ...
        // <ces:GetCurrencies>
        SOAPElement getCurrenciesElem = soapBody.addChildElement("GetEnumerationValues", cesNamespace);
        ...
        //  <Request EnumType="IDTypes" CountryFrom="CL"/>
        SOAPElement requestElement = root.addChildElement("Request");

        requestElement.addAttribute(new QName("", "EnumType"), request.getEnumType());
                        ...
                        while (it4.hasNext()) {
                            ...
                            if ("RequestResponses".equals(element4.getElementName().getLocalName())) {
                                ...
                                while (it5.hasNext()) {
                                    ...
                                    String inputParams = element5.getAttribute("InputParams");
                                    Iterator it6 = element5.getChildElements();
                                    while (it6.hasNext()) {
                                        SOAPElement element7 = (SOAPElement) it6.next();
                                        String value = element7.getAttribute("Value");
                                        String name = element7.getAttribute("Name");
                                        String description = element7.getAttribute("Description");
                                        enumerationValuesList.add(value + "|" + name + "|" + description + "|");
                                    }
                                }
                            }
                        }
    ...
    private static SOAPMessage createSOAPRequest(String soapAction,
            GetEnumerationValuesRequestEntity request) throws Exception {
