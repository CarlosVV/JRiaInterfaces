using CES.CoreApi.Compliance.Alert.Filters.Responses;
using CES.CoreApi.Compliance.Alert.Utilities;
...
namespace CES.CoreApi.Compliance.Alert.Filters
{
	public class HttpMessageHandler : DelegatingHandler
	{
		...
		private HttpResponseMessage BuildApiResponse(HttpRequestMessage request, HttpResponseMessage response)
		{
			...
			var newResponse = request.CreateResponse(response.StatusCode, new ErrorResponse("Invalid request or required fields was not provided", 400
