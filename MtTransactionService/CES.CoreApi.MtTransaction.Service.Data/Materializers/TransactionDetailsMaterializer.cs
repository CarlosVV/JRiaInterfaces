...
{
    public class TransactionDetailsMaterializer : ITransactionDetailsMaterializer
    {
        #region Core
        ...
        #region ITransactionDetailsMaterializer implementation

        public TransactionDetailsModel GetTransactionDetails(IDataReader reader, InformationGroup detalizationLevel)
        {
            var response = new TransactionDetailsModel
            {
            ...
            if ((detalizationLevel & InformationGroup.Customer) == InformationGroup.Customer)
                response.Customer = GetCustomerDetails(reader);

            if ((detalizationLevel & InformationGroup.Beneficiary) == InformationGroup.Beneficiary)
                response.Beneficiary = GetBeneficiaryDetails(reader);

            if ((detalizationLevel & InformationGroup.ComplianceInformation) == InformationGroup.ComplianceInformation)
                response.ComplianceInformation = GetComplianceDetails(reader);

            if ((detalizationLevel & InformationGroup.MoneyTransferDetails) == InformationGroup.MoneyTransferDetails)
                response.MoneyTransferDetails = GetMoneyTransferDetails(reader);

            if ((detalizationLevel & InformationGroup.PayingAgent) == InformationGroup.PayingAgent)
                response.PayingAgent = GetPayingAgentDetails(reader);

            if ((detalizationLevel & InformationGroup.ReceivingAgent) == InformationGroup.ReceivingAgent)
                response.ReceivingAgent = GetReceivingAgentDetails(reader);

            if ((detalizationLevel & InformationGroup.TransactionStatus) == InformationGroup.TransactionStatus)
                response.TransactionStatus = GetTransactionStatusDetails(reader);

            if ((detalizationLevel & InformationGroup.ProcessingInformation) == InformationGroup.ProcessingInformation)
                response.ProcessingInformation = GetProcessingInformation(reader);
        ...
        private static MoneyTransferDetailsModel GetMoneyTransferDetails(IDataReader reader)
        {
            ...
            {
                PreId = reader.ReadValue<int>("fPreOrderTransID"),
        ...
        private static MonetaryInformationModel GetMonetaryInformation(IDataReader reader)
        {
            return new MonetaryInformationModel
            {
                ...
                {
                    TransactionAmount = reader.ReadValue<decimal>("OrderAmount"),
        ...
        private static ContactModel GetContactDetails(IDataReader reader, string homePhoneFieldName, string cellPhoneFieldName)
        {
            ...
            var response = new ContactModel { PhoneList = new Collection<TelephoneModel>() };
