...
using CES.CoreApi.Accounting.Service.Business.Logic.Processors;
using CES.CoreApi.Accounting.Service.Contract.Models;
using CES.CoreApi.Accounting.Service.Data.Repositories;
using CES.CoreApi.Accounting.Service.Interfaces;
using CES.CoreApi.Accounting.Service.Utilities;
...
namespace CES.CoreApi.Accounting.Service.Configuration
{
    internal class CompositionRoot
    {
        ...
        {
            container.RegisterSingle<ITransactionInformationProcessor, TransactionInformationProcessor>();
        ...
        private static void RegisterInterceptions(Container container)
        {
            ...
            container.InterceptWith<PerformanceInterceptor>(type => type == typeof(ITransactionInformationRepository));
            container.InterceptWith<PerformanceInterceptor>(type => type == typeof(ITransactionInformationProcessor));
        }

        private static void RegisterResponses(Container container)
        {
            container.Register<GetTransactionSummaryResponse, GetTransactionSummaryResponse>();
