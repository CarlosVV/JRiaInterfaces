...
namespace CES.CoreApi.GeoLocation.Logic.Builders
{
    ...
    //            maxRecords);
			...
			var url =$"{GeoLocationConfigurationSection.Instance.MelissaData.Url}?id={key}&format=xml&Address1={HttpUtility.UrlEncode(address.Address1)}&AdministrativeArea={HttpUtility.UrlEncode(address.AdministrativeArea)}&Locality={HttpUtility.UrlEncode(address.City)}&Country={HttpUtility.UrlEncode(address.Country)}&PostalCode={HttpUtility.UrlEncode(address.PostalCode)}&MaxRecords={maxRecords}";
        ...
        public string BuildUrl(AddressModel address)
        {
            var url = string.Format(
				"{0}?format=json&id={1}&a1={2}&a2={3}&loc={4}&admarea{5}&postal={6}&ctry={7}&opt=DeliveryLines:ON",
